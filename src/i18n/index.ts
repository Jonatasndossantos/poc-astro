/**
 * i18n utility — loads content from src/content/<key>/<locale>.json
 *
...
/**
 * Returns the translated content for a given locale and namespace key.
 ...
 * @param key       - Concept path inside src/content/ (e.g. 'fullstack/hero', 'shared/nav')
 */
export async function getTranslations<T extends FlatDictionary = FlatDictionary>(
    ...
    try {
        const file = await import(`../content/${key}/${locale}.json`);
        return file.default as T;
    } catch {
        // Fallback to English if the locale file doesn't exist yet
        const fallback = await import(`../content/${key}/en.json`);
